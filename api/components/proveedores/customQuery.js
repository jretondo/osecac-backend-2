...
const getList = (table, busqueda, tipo, cbu) => {
    ...
    return ` SELECT * FROM ${table} WHERE (raz_soc LIKE '%${busqueda}%' OR localidad LIKE '%${busqueda}%' OR encargado LIKE '%${busqueda}%' OR email LIKE '%${busqueda}%') AND tipo_prov = '${tipo}' ${tieneCbu} `
}
